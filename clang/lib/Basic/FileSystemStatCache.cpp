...
FileSystemStatCache::get(StringRef Path, llvm::vfs::Status &Status,
  ...
  if (Cache)
    ...
    DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": calling FS.status(" << Path.str() << ")\n");
    llvm::ErrorOr<llvm::vfs::Status> StatusOrErr = FS.status(Path);
    if (!StatusOrErr) {
      DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": !StatusOrErr\n");
      RetCode = StatusOrErr.getError();
    } else {
      DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": StatusOrErr is non-null\n");
      Status = *StatusOrErr;
    }
  } else {
    ...
    DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": calling openFileForRead(" << Path.str() << "\n");
    auto OwnedFile = FS.openFileForRead(Path);
    DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": returned from openFileForRead(" << Path.str() << "\n");

    if (!OwnedFile) {
      DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": !OwnedFile\n");
      // If the open fails, our "stat" fails.
      RetCode = OwnedFile.getError();
    } else {
      DYN_DEBUG(remotefs_debug, llvm::errs() << __FILE__  << " " << __LINE__ << " " << __func__ << ": OwnedFile is non-null\n");
