...
void clang::ParseAST(Sema &S, bool PrintStats, bool SkipFunctionBodies) {
  ...
  llvm::errs() << __func__ << " " << __LINE__ << ": calling EnterMainSourceFile().\n";
  S.getPreprocessor().EnterMainSourceFile();
  llvm::errs() << __func__ << " " << __LINE__ << ": done calling EnterMainSourceFile().\n";

  ExternalASTSource *External = S.getASTContext().getExternalSource();
  if (External) {
    llvm::errs() << __func__ << " " << __LINE__ << ": calling StartTranslationUnit().\n";
    External->StartTranslationUnit(Consumer);
    llvm::errs() << __func__ << " " << __LINE__ << ": done calling StartTranslationUnit().\n";
  }
  ...
  if (HaveLexer) {
    ...
    llvm::errs() << __func__ << " " << __LINE__ << ": calling Initialize().\n";
    P.Initialize();
    llvm::errs() << __func__ << " " << __LINE__ << ": done calling Initialize().\n";
    Parser::DeclGroupPtrTy ADecl;
    Sema::ModuleImportState ImportState;
    EnterExpressionEvaluationContext PotentiallyEvaluated(
        S, Sema::ExpressionEvaluationContext::PotentiallyEvaluated);

    llvm::errs() << __func__ << " " << __LINE__ << ": calling ParseFirstTopLevelDecl().\n";
    ...
    for (bool AtEOF = P.ParseFirstTopLevelDecl(ADecl, ImportState); !AtEOF;
      ...
      // skipping something.
      llvm::errs() << __func__ << " " << __LINE__ << ": calling HandleTopLevelDecl().\n";
  ...
  for (Decl *D : S.WeakTopLevelDecls()) {
    ...
    llvm::errs() << __func__ << " " << __LINE__ << ": done calling HandleTopLevelDecl().\n";
  }

  llvm::errs() << __func__ << " " << __LINE__ << ": calling HandleTranslationUnit()\n";
  Consumer->HandleTranslationUnit(S.getASTContext());
  llvm::errs() << __func__ << " " << __LINE__ << ": done calling HandleTranslationUnit()\n";
