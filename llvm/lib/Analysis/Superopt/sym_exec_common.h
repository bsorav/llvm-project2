...
public:
  ...
  static pair<map<symbol_id_t, graph_symbol_t>, map<pair<symbol_id_t, offset_t>, vector<char>>> get_symbol_map_and_string_contents(llvm::Module const *M, list<pair<string, unsigned>> const &fun_names, dshared_ptr<tfg const> src_tfg);
  static vector<char> get_constant_bytes(llvm::Constant const* c);
  static map<symbol_id_t, graph_symbol_t> sym_exec_get_symbol_map(llvm::Module const *M, dshared_ptr<tfg const> src_tfg);
  static map<pair<symbol_id_t, offset_t>, vector<char>> get_string_contents(llvm::Module const *M, dshared_ptr<tfg const> src_tfg);
  ...
  static symbol_id_t get_symbol_id_for_name(string const& name, dshared_ptr<tfg const> src_tfg, symbol_id_t input_symbol_id);
