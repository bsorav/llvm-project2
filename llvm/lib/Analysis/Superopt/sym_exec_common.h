...
public:
  ...
  static pair<graph_symbol_map_t, map<pair<symbol_id_t, offset_t>, vector<char>>> get_symbol_map_and_string_contents(llvm::Module const *M, list<pair<string, unsigned>> const &fun_names, dshared_ptr<tfg const> src_tfg);
  static graph_symbol_map_t get_symbol_map(llvm::Module const *M, dshared_ptr<tfg const> src_tfg);
