...
function2tfg(Function *F, Module *M, map<shared_ptr<tfg_edge const>, Instruction const*>& eimap)
{
  ...
  dshared_ptr<tfg_llvm_t> ret = sym_exec_llvm::get_tfg(*F, M, F->getName().str(), ctx, dshared_ptr<tfg_llvm_t const>::dshared_nullptr(), false /*model_llvm_semantics*/, nullptr/*, nullptr*/, eimap, {}, srcdst_t::srcdst_src, dshared_ptr<ll_filename_parsed_t>::dshared_nullptr(), points_to_algo_t::points_to_algo_dummy()/*, context::XML_OUTPUT_FORMAT_TEXT_NOCOLOR*/, compiler_id_t::unknown);
  pc_ref start_pc = sym_exec_llvm::get_start_pc(ret->get_srcdst(), *F);
