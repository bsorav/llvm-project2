...
sym_exec_common::get_symbol_map_and_string_contents(Module const *M, list<pair<string, unsigned>> const &fun_names, dshared_ptr<tfg_llvm_t const> src_llvm_tfg)
{
  ...
  for (auto &g : M->getGlobalList()) {
    ...
    bool inserted = smap.insert(make_pair(symbol_id, graph_symbol_t(mk_string_ref(name), symbol_size, symbol_alignment, symbol_is_constant, symbol_is_function))).second;
    ASSERT(inserted);
  ...
  for (const auto &fun_name : fun_names) {
    ...
    bool inserted = smap.insert(make_pair(symbol_id, graph_symbol_t(mk_string_ref(fun_name.first), fun_name.second, ALIGNMENT_FOR_FUNCTION_SYMBOL, false, true))).second;
    ASSERT(inserted);
    symbol_id++;
  }
