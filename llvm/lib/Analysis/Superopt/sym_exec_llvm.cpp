...
#include "expr/llvm_reg_src_info.h"
...
expr_var_ref prefix_identifier(const expr_var_ref& id, const string& prefix)
{
  //return prefix + "." + id;
  return id->expr_var_set_name(prefix + "." + id->get_name()->get_str());
}
...
{
  expr_ref ret;
  for (auto const& [symbol_id, sym] : *m_symbol_map) {
  ...
  for (auto const& [symbol_id, sym] : *m_extsym_map) {
    if (sym.get_name()->get_str() == name) {
      ostringstream ss;
      //ss << "symbol." << symbol_id;
      ss << G_SYMBOL_KEYWORD "." << symbol_id;
      ret = m_ctx->mk_var(expr_var_t::mk_expr_var_extsym(mk_string_ref(ss.str()), symbol_id, sym), sr);
      m_touched_extsyms.insert(symbol_id);
      return ret;
    }
  }
  return ret;
...
sym_exec_llvm::get_const_value_expr(const llvm::Value& v, expr_var_ref/*string*/ const& vname, const state& state_in, preds_t const& state_assumes, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  ...
  else if (ConstantExpr const* ce = (ConstantExpr const*)dyn_cast<const ConstantExpr>(&v))
  {
    ...
    shared_ptr<tfg_edge const> e = mk_tfg_edge(from_node->get_pc(), intermediate_node->get_pc(), expr_true(m_ctx), state_to_intermediate_val, {}, {}, this->instruction_to_te_comment(*i_sp, from_node->get_pc()));
    t.add_edge(e);
  ...
  else if(const GlobalValue* c = dyn_cast<const GlobalValue>(&v))
  {
    ...
    ASSERT(!ret);
    ret = get_symbol_expr_for_global_var(name->get_name()->get_str(), sr);
    if (!ret) {
      ret = m_ctx->mk_var(name->expr_var_set_name(string(LLVM_FUNCTION_NAME_PREFIX) + name->get_name()->get_str()), get_value_type(v, m_module->getDataLayout()));
    }
    ASSERT(ret);
  ...
  {
    expr_var_ref undef_key/*_name*/ = get_next_undef_varname();
    expr_var_ref undef_poison_var/*name*/ = get_poison_value_varname(undef_key_name);

    sort_ref s = get_value_type(v, m_module->getDataLayout());

    state state_to_intermediate_val;
    //expr_var_ref undef_poison_var = expr_var_t::mk_expr_var_plain(mk_string_ref(undef_poison_varname));
    //expr_var_ref undef_key = expr_var_t::mk_expr_var_plain(mk_string_ref(undef_key_name));
    state_set_expr(state_to_intermediate_val, undef_poison_var, expr_true(m_ctx));
    state_set_expr(state_to_intermediate_val, undef_key, m_ctx->mk_var(LLVM_UNDEF_VALUE_NAME, s));
    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    pc_ref from_pc = from_node->get_pc();
    shared_ptr<tfg_edge const> e = mk_tfg_edge(from_pc, intermediate_node->get_pc(), expr_true(m_ctx), state_to_intermediate_val, {}, {}, te_comment_t(false, from_pc->get_subindex(), "treat-undef-as-poison"));
    t.add_edge(e);
    from_node = intermediate_node;

    m_poison_varnames_seen.insert(undef_poison_var);
    expr_ref undef_expr = mk_fresh_expr(undef_key, G_INPUT_KEYWORD, s);
  ...
  expr_ref ret = m_ctx->mk_var(unsupported_value_varname->expr_var_set_name("unsupported." + unsupported_value_name), get_value_type(v, m_module->getDataLayout()));
...
expr_ref sym_exec_common::mk_fresh_expr(expr_var_ref const& name, const string& prefix, sort_ref s) const
{
  expr_var_ref id = prefix_identifier(name, prefix);
...
string
sym_exec_llvm::float_fcall_arg_get_name(CallInst const& I, int argnum) const
{
...
sym_exec_llvm::populate_state_template(const llvm::Function& F, bool model_llvm_semantics)
{
  ...
  for (auto const& v : F.args()) {
    ...
    if (v.hasByValAttr()) {
      ...
      expr_ref argvar = m_ctx->mk_var(name->expr_var_add_input_prefix(), s);
      m_arguments[name->get_name()->get_str()] = make_pair(argnum, argvar);
      m_local_refs.insert(make_pair(allocsite, graph_local_t(name->get_name()->get_str(), size, align)));
    }
    argnum++;
    ...
    if (model_llvm_semantics) {
      expr_var_ref arg_poison_varname = get_poison_value_varname(name);
      expr_ref arg_poison_var = m_ctx->mk_var(arg_poison_varname->expr_var_add_input_prefix(), m_ctx->mk_bool_sort());
      m_arguments[arg_poison_varname->get_name()->get_str()] = make_pair(argnum, arg_poison_var);
      allocsite_t allocsite = allocsite_t::allocsite_arg(argnum);
      argnum++;
      m_poison_varnames_seen.insert(arg_poison_varname);
      m_local_refs.insert(make_pair(allocsite, graph_local_t(arg_poison_varname->get_name()->get_str(), 1, 0)));
    }
  }
  if (F.isVarArg()) {
...
void sym_exec_common::get_state_template(pc_ref const& p, state& st)
{
  ...
  for (const pair<expr_var_ref, sort_ref>& item : m_state_templ) {
    expr_var_ref name = item.first;
    //if (m_value_name_map.count(name) > 0) {
    //  name = m_value_name_map.at(name);
    //}
    sort_ref s = item.second;
    value_expr_map.insert(make_pair(name, mk_fresh_expr(name, prefix, s)));
  }
  st.set_value_expr_map(value_expr_map);
}

pair<expr_ref,preds_t>
sym_exec_llvm::get_expr_adding_edges_for_intermediate_vals(const Value& v, expr_var_ref/*string*/ const& vname, const state& state_in, preds_t const& state_assumes, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, tfg& t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
...
//{
//  state_set_expr(st, expr_var_t::mk_expr_var_plain(mk_string_ref(key)), value);
//}

void
sym_exec_common::state_set_expr(state &st, expr_var_ref const &key, expr_ref const &value) const
{
...
//{
//  return get_input_expr(expr_var_t::mk_expr_var_plain(mk_string_ref(key)), s);
//}

expr_ref
sym_exec_common::get_input_expr(expr_var_ref const &key, sort_ref const& s) const
{
  return m_ctx->mk_var(key->expr_var_add_input_prefix(), s);
}

//expr_ref
//sym_exec_common::state_get_expr(state const &st, string const &key, sort_ref const& s) const
//{
//  return state_get_expr(st, expr_var_t::mk_expr_var_plain(mk_string_ref(key)), s);
//}

expr_ref
sym_exec_common::state_get_expr(state const &st, expr_var_ref const &key, sort_ref const& s) const
{
  if (st.has_expr(key)) {
...
sym_exec_llvm::get_expr_args(const llvm::Instruction& I, expr_var_ref/*string*/ const& vname, const state& st, preds_t const& state_assumes, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
...
pair<expr_ref,preds_t>
sym_exec_llvm::exec_gen_expr_casts(const llvm::CastInst& I, expr_ref arg, preds_t const& state_assumes, pc_ref const &from_pc, bool model_llvm_semantics, tfg &t)
{
...
sym_exec_llvm::apply_memset_function(const CallInst* c, expr_ref fun_name_expr, string const &fun_name, dshared_ptr<tfg_llvm_t const> src_llvm_tfg, Function *F, state const &state_in, state &state_out, preds_t const& state_assumes, string const &cur_function_name, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, llvm::Function const &curF, tfg &t/*, map<string, pair<callee_summary_t, dshared_ptr<tfg_llvm_t>>> *function_tfg_map*/, map<llvm_value_id_t, expr_var_ref>* value_to_name_map/*, set<string> const *function_call_chain*/, map<string, value_scev_map_t> const& scev_map/*, context::xml_output_format_t xml_output_format*/)
{
  ...
  if (memset_nbytes_expr->is_const()) {
    ...
    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    shared_ptr<tfg_edge const> e = mk_tfg_edge(cur_pc, intermediate_node->get_pc(), expr_true(m_ctx), state_out, assumes, {}, this->instruction_to_te_comment(*c, from_pc));
    t.add_edge(e);
...
sym_exec_llvm::apply_memcpy_function(const CallInst* c, expr_ref fun_name_expr, string const &fun_name, dshared_ptr<tfg_llvm_t const> src_llvm_tfg, Function *F, state const &state_in, state &state_out, preds_t const& state_assumes, string const &cur_function_name, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, llvm::Function const &curF, tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map, map<string, value_scev_map_t> const& scev_map/*, context::xml_output_format_t xml_output_format*/)
{
  ...
  if (memcpy_nbytes_expr->is_const()) {
    ...
    if (count != 0) {
      add_state_assume("", expr_with_fail(gen_is_aligned_assume_expr(memcpy_src_expr, memcpy_src_align), fails::safety_memcpy_src_aligned), state_in, assumes, from_node, model_llvm_semantics, t, value_to_name_map);
      add_state_assume("", expr_with_fail(gen_is_aligned_assume_expr(memcpy_dst_expr, memcpy_dst_align), fails::safety_memcpy_tgt_aligned), state_in, assumes, from_node, model_llvm_semantics, t, value_to_name_map);
    }
    pc_ref cur_pc = from_node->get_pc();
    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    shared_ptr<tfg_edge const> e = mk_tfg_edge(cur_pc, intermediate_node->get_pc(), expr_true(m_ctx), state_out, assumes, {}, this->instruction_to_te_comment(*c, from_pc));
...
sym_exec_llvm::apply_fmuladd_function(const CallInst* c, expr_ref fun_name_expr, string const &fun_name, dshared_ptr<tfg_llvm_t const> src_llvm_tfg, Function *F, state const &state_in, state &state_out, preds_t const& state_assumes, string const &cur_function_name, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, llvm::Function const &curF, tfg &t/*, map<string, pair<callee_summary_t, dshared_ptr<tfg_llvm_t>>> *function_tfg_map*/, map<llvm_value_id_t, expr_var_ref>* value_to_name_map/*, set<string> const *function_call_chain*/, map<string, value_scev_map_t> const& scev_map/*, context::xml_output_format_t xml_output_format*/)
{
  ...
  tie(fmuladd_a_expr, assumes)    = get_expr_adding_edges_for_intermediate_vals(*fmuladd_a, "", state_out, assumes, from_node, model_llvm_semantics, t, value_to_name_map);
  tie(fmuladd_b_expr, assumes)    = get_expr_adding_edges_for_intermediate_vals(*fmuladd_b, "", state_out, assumes, from_node, model_llvm_semantics, t, value_to_name_map);
  tie(fmuladd_c_expr, assumes)    = get_expr_adding_edges_for_intermediate_vals(*fmuladd_c, "", state_out, assumes, from_node, model_llvm_semantics, t, value_to_name_map);
  ...
  //  shared_ptr<tfg_edge const> e = mk_tfg_edge(cur_pc, intermediate_node->get_pc(), expr_true(m_ctx), state_out, assumes, {}, this->instruction_to_te_comment(*c, from_pc));
  //  t.add_edge(e);
  //  DYN_DEBUG(llvm2tfg_memcpy, cout << "added no-op edge " << e->to_string_concise() << " for fmuladd" << endl);
  ...
  //  t.add_edge(e);
  //  DYN_DEBUG(llvm2tfg_memcpy, cout << "added edge " << e->to_string_concise() << " for fmuladd" << endl);
  //  cur_pc = intermediate_node->get_pc();
  //}
...
sym_exec_llvm::apply_statement_marker_function(const CallInst* c, state &state_out, preds_t const& state_assumes, code_marker_fn_t marker_fn)
{
  ...
  state_set_expr(state_out, expr_var_t::mk_expr_var_plain(mk_string_ref(code_marker_reg)),
      (m_ctx->*marker_fn)(
          in_code_marker,
...
sym_exec_llvm::apply_va_start_function(const CallInst* c, state const& state_in, state &state_out, preds_t const& state_assumes, string const& cur_function_name, dshared_ptr<tfg_node const>& from_node, bool model_llvm_semantics, tfg& t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  ...
  return make_pair(assumes, preds_t());
}
  ...
  for (const auto& arg : c->args()) {
    ...
    if (c->paramHasAttr(argnum, Attribute::ByVal)) {
      ...
      if (callconv_is_cdecl_x86(*c)) {
        ...
        state interm_st;
        state_set_expr(interm_st, expr_var_t::mk_expr_var_plain(mk_string_ref(byval_expr_name)), byval_expr_val);
        dshared_ptr<tfg_node const> interm_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
        pc_ref const& from_pc = from_node->get_pc();
        ...
        expr = m_ctx->mk_var(expr_var_t::mk_expr_var_plain(mk_string_ref(byval_expr_name))->expr_var_add_input_prefix(), byval_expr_val->get_sort());
      } else {
    ...
    if (model_llvm_semantics) {
      if (m_ctx->expr_is_input_expr(expr)) {
        expr_var_ref val_key = expr->expr_get_var()->expr_var_remove_input_prefix();
        expr_var_ref val_key_poison = get_poison_value_varname(val_key);

        if (set_belongs(m_poison_varnames_seen, val_key_poison)) {
          args.push_back(m_ctx->mk_var(val_key_poison->expr_var_add_input_prefix(), m_ctx->mk_bool_sort()));
        } else {
...
expr_ref
fp_val_within_limits_assume(expr_ref const& e, float_max_t const& min_limit, float_max_t const& max_limit)
{
  context* ctx = e->get_context();
  expr_ref ge_min = ctx->mk_fcmp_oge(e, ctx->mk_float_const(e->get_sort()->get_size(), min_limit));
  expr_ref lt_max = ctx->mk_fcmp_olt(e, ctx->mk_float_const(e->get_sort()->get_size(), max_limit));
...
void sym_exec_llvm::exec(const state& state_in, const llvm::Instruction& I, dshared_ptr<tfg_node const> from_node, llvm::BasicBlock const &B, llvm::Function const &F, pc_ref const& pc_to, dshared_ptr<tfg_llvm_t const> src_llvm_tfg, bool model_llvm_semantics, tfg_llvm_t &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map/*, set<string> const *function_call_chain*/, map<shared_ptr<tfg_edge const>, Instruction const*>& eimap, map<string, value_scev_map_t> const& scev_map/*, context::xml_output_format_t xml_output_format*/, map<string, bool>& nextpc_is_noreturn_map)
{
  ...
  case Instruction::Ret:
  {
    ...
    if(Value* ret = i->getReturnValue())
    {
      ...
      state_set_expr(state_out, expr_var_t::mk_expr_var_plain(mk_string_ref(::get_srcdst_keyword(m_srcdst) + "." + G_LLVM_RETURN_REGISTER_NAME))/*m_ret_reg*/, dst_val);
    }
    for (size_t i = 0; i < LLVM_NUM_CALLEE_SAVE_REGS; i++) {
      stringstream ss;
  ...
  case Instruction::Alloca:
  {
    ...
    ASSERT(a);
    string const iname = get_value_name(*a)->get_name()->get_str();
    ...
    if (dilocal && this->alloca_corresponds_to_a_local_parameter(*a, *dilocal, F, t, param_addr)) {
      ...
      expr_ref const local_alloc_count_var = state_get_expr(state_in, expr_var_t::mk_expr_var_plain(mk_string_ref(local_alloc_count_varname)), m_ctx->mk_count_sort());
      expr_ref const local_alloc_count_ssa_var = state_get_expr(state_in, expr_var_t::mk_expr_var_plain(mk_string_ref(local_alloc_count_ssa_varname)), m_ctx->mk_count_sort());
      ...
      state_set_expr(state_out, local_size_var->expr_get_var()->expr_var_remove_input_prefix(), local_size_val);
      add_edge_with_state();
      ...
      if (m_ctx->get_config().prefer_friendly_counterexamples) {
        ...
        add_state_assume(iname, alloc_ptr_expr_ne_zero, state_in, state_assumes, from_node, model_llvm_semantics, t, value_to_name_map);
      }
      add_edge_with_state();
  ...
  case Instruction::Store:
  {
    ...
    add_state_assume("", expr_with_fail(gen_dereference_assume_expr(addr), fails::safety_store_nonull_deref), state_in, state_assumes, from_node, model_llvm_semantics, t, value_to_name_map);
    ...
    if (align != 0) {
      add_state_assume("", expr_with_fail(gen_is_aligned_assume_expr(addr, align),fails::safety_store_aligned), state_in, state_assumes, from_node, model_llvm_semantics, t, value_to_name_map);
    }
  ...
  case Instruction::Call:
  {
    ...
    if(c->hasStructRetAttr()) {
      t.set_fcall_pc_return_via_hidden_ptr(from_node->get_pc());
    }
    ...
    preds_t heap_allocfree_assumes;
  ...
  case Instruction::FRem:
  {
    ...
    string iname = get_value_name(I)->get_name()->get_str();
...
te_comment_t
sym_exec_common::instruction_to_te_comment(llvm::Instruction const& I, pc_ref const& from_pc/*, bbl_order_descriptor_t const& bbo*/) const
{
  string s;
  raw_string_ostream ss(s);
...
pair<expr_ref,preds_t>
sym_exec_llvm::exec_gen_expr(const llvm::Instruction& I, string const& Iname, const vector<expr_ref>& args, state const &state_in, preds_t const& state_assumes, dshared_ptr<tfg_node const> &from_node, bool model_llvm_semantics, tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  ...
  case Instruction::GetElementPtr:
  {
    ...
    for(gep_type_iterator itype = itype_begin; itype != itype_end; ++itype, ++index_counter)
    {
      ...
      state state_to_intermediate_val;
      expr_var_ref offset_var = expr_var_t::mk_expr_var_plain(mk_string_ref(offset_name));
      expr_var_ref total_offset_var = expr_var_t::mk_expr_var_plain(mk_string_ref(total_offset_name));
      state_set_expr(state_to_intermediate_val, offset_var, offset_expr);
      state_set_expr(state_to_intermediate_val, total_offset_var, new_cur_expr);
      ...
      dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
      shared_ptr<tfg_edge const> e = mk_tfg_edge(cur_pc, intermediate_node->get_pc(), expr_true(m_ctx), state_to_intermediate_val, assumes, {}, this->instruction_to_te_comment(I, from_node->get_pc()/*, bbo*/));
      t.add_edge(e);

      cur_expr = mk_fresh_expr(total_offset_var, G_INPUT_KEYWORD, m_ctx->mk_bv_sort(get_word_length()));
      cur_pc = intermediate_node->get_pc();
    }
    ...
    state state_to_intermediate_val;
    expr_var_ref total_offset_var = expr_var_t::mk_expr_var_plain(mk_string_ref(total_offset_name));
    state_set_expr(state_to_intermediate_val, total_offset_var, m_ctx->mk_bvadd(ptr, cur_expr));

    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    shared_ptr<tfg_edge const> e = mk_tfg_edge(cur_pc, intermediate_node->get_pc(), expr_true(m_ctx), state_to_intermediate_val, {}, {}, this->instruction_to_te_comment(I, from_node->get_pc()/*, bbo*/));
    t.add_edge(e);
    ...
    return make_pair(cur_expr, preds_t());
  }
  default:
    errs() << I << "\n";
...
sym_exec_llvm::sym_exec_populate_tfg_scev_map(tfg_llvm_t& t_src, value_scev_map_t const& value_scev_map) const
{
  for (auto const& vs : value_scev_map) {
    ...
    scev_toplevel_t<pc_ref> const& scev_toplevel = vs.second;
...
sym_exec_llvm::expand_switch(tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map, dshared_ptr<tfg_node const> const &from_node, vector<control_flow_transfer> const &cfts, state const &state_to, preds_t const& assumes, te_comment_t const& te_comment, llvm::Instruction const* I, const llvm::BasicBlock& B, const llvm::Function& F, map<shared_ptr<tfg_edge const>, llvm::Instruction const*>& eimap)
{
  ...
  pc_ref from_pc = from_node->get_pc();
  dshared_ptr<tfg_node const> cur_node = from_node;
  size_t varnum = 0;
  for (size_t i = 0; i < cfts.size() - 1; i++) {
    ...
    pc_ref to_pc = cft.get_to_pc();
    if (!t.find_node(to_pc)) {
      t.add_node(make_dshared<tfg_node>(to_pc));
    }
    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, cur_node);
    pc_ref const& intermediate_pc = intermediate_node->get_pc();
    dshared_ptr<tfg_node const> next_case_node = get_next_intermediate_subsubindex_pc_node(t, intermediate_node);
    ...
    expr_ref switch_tmpvar = get_input_expr(expr_var_t::mk_expr_var_plain(mk_string_ref(switch_tmpvar_name)), m_ctx->mk_bool_sort());

    // edge for setting switch tmpvar
    preds_t assumes = cft.get_assumes();
    preds_union(assumes, std::move(cond_assumes));
    state state_to_newvar;
    state_set_expr(state_to_newvar, expr_var_t::mk_expr_var_plain(mk_string_ref(switch_tmpvar_name)), cft.get_condition());
    shared_ptr<tfg_edge const> e1 = mk_tfg_edge(cur_node->get_pc(), intermediate_pc, expr_true(m_ctx), state_to_newvar, assumes, {}, te_comment);
    t.add_edge(e1);
    DYN_DEBUG2(llvm2tfg, cout << __func__ << ':' << __LINE__ << ": Added edge e1 = " << e1->to_string() << endl);

    // edge for ~case jump
    shared_ptr<tfg_edge const> enp = mk_tfg_edge(intermediate_pc, next_case_pc, m_ctx->mk_not(switch_tmpvar), state_to, {}, {}, te_comment);
    t.add_edge(enp);
...
sym_exec_llvm::process_cft(tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map, dshared_ptr<tfg_node const> const &from_node, pc_ref const &pc_to, expr_ref target, expr_ref to_condition, state const &state_to, preds_t const& assumes, bool model_llvm_semantics, te_comment_t const& te_comment, Instruction const* I, const llvm::BasicBlock& B, const llvm::Function& F, map<shared_ptr<tfg_edge const>, Instruction const*>& eimap)
{
  ...
  if (target) {
    state_set_expr(state_to_cft, expr_var_t::mk_expr_var_plain(mk_string_ref(::get_srcdst_keyword(m_srcdst) + "." LLVM_STATE_INDIR_TARGET_KEY_ID)), target);
...
sym_exec_llvm::process_cfts(tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map, dshared_ptr<tfg_node const> const &from_node, pc_ref const &pc_to, state const &state_to, preds_t const& state_assumes, bool model_llvm_semantics, te_comment_t const& te_comment, Instruction const* I, vector<control_flow_transfer>&& cfts, llvm::BasicBlock const &B, llvm::Function const &F, map<shared_ptr<tfg_edge const>, Instruction const*>& eimap)
{
  ...
  if (cfts.size() == 0) {
    process_cft(t, value_to_name_map, from_node, pc_to, nullptr, expr_true(m_ctx), state_to, state_assumes, model_llvm_semantics, te_comment, I, B, F, eimap);
  } else {
    for(control_flow_transfer& cft : cfts) {
      //first half performs all the computation required in the successor phi nodes, as part of the current basic block
      preds_t assumes = state_assumes;
      preds_t cft_assumes = cft.get_assumes();
      preds_union(assumes, std::move(cft_assumes));
      dshared_ptr<tfg_node const> cft_from_node = t.find_node(cft.get_from_pc());
      ASSERT(cft_from_node);
      process_cft(t, value_to_name_map, cft_from_node, cft.get_to_pc(), cft.get_target(), cft.get_condition(), state_to, assumes, model_llvm_semantics, te_comment, I, B, F, eimap);
    }
  }
}

void
sym_exec_llvm::parse_dbg_declare_intrinsic(Instruction const& I, tfg_llvm_t& t, pc_ref const& pc_from) const
{
...
void
sym_exec_llvm::parse_dbg_value_intrinsic(Instruction const& I, tfg_llvm_t& t, pc_ref const& pc_from) const
{
  const CallInst& CI = cast<CallInst>(I);
...
state
sym_exec_llvm::parse_stacksave_intrinsic(Instruction const& I, tfg& t, pc_ref const& pc_from)
{
  const CallInst& CI = cast<CallInst>(I);
  ...
  state_set_expr(state_out, expr_var_t::mk_expr_var_plain(memalloc_ssa_varname), mem_alloc_e);
  return state_out;
}

state
sym_exec_llvm::parse_stackrestore_intrinsic(Instruction const& I, tfg& t, pc_ref const& pc_from)
{
  const CallInst& CI = cast<CallInst>(I);
  state state_in, state_out;
...
sym_exec_llvm::add_edges(const llvm::BasicBlock& B, dshared_ptr<tfg_llvm_t const> src_llvm_tfg, bool model_llvm_semantics, tfg_llvm_t& t, const llvm::Function& F/*, map<string, pair<callee_summary_t, dshared_ptr<tfg_llvm_t>>> *function_tfg_map*/, map<llvm_value_id_t, expr_var_ref>* value_to_name_map/*, set<string> const *function_call_chain*/, map<shared_ptr<tfg_edge const>, Instruction const*>& eimap, map<string, value_scev_map_t> const& scev_map, dshared_ptr<ll_filename_parsed_t> const& ll_filename_parsed/*, context::xml_output_format_t xml_output_format*/, map<string, bool>& nextpc_is_noreturn_map)
{
  ...
  for (const Instruction& I : B) {
    ...
    dshared_ptr<tfg_node const> from_node = make_dshared<tfg_node>(pc_from);
...
sym_exec_llvm::get_basic_block_for_pc(const llvm::Function& F, pc_ref const &p)
{
  stringstream ss;
...
dshared_ptr<tfg_node const>
sym_exec_common::get_next_intermediate_subsubindex_pc_node(tfg &t, dshared_ptr<tfg_node const> const &from_node)
{
  auto const& from_pc = from_node->get_pc();
  char const *index = from_pc->get_index();
...
sym_exec_llvm::process_phi_nodes(tfg &t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map, const llvm::BasicBlock* B_from, pc_ref const& pc_to, dshared_ptr<tfg_node const> const &from_node, bool model_llvm_semantics, expr_ref edgecond, preds_t const& assumes, te_comment_t const& te_comment, Instruction const* I, const llvm::Function& F, map<shared_ptr<tfg_edge const>, Instruction const*>& eimap)
{
  ...
  dshared_ptr<tfg_node const> pc_to_phi_node = from_node;

  dshared_ptr<tfg_node const> pc_to_phi_start_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
  auto e1 = mk_tfg_edge(pc_to_phi_node->get_pc(), pc_to_phi_start_node->get_pc(), edgecond, state(), assumes, {}, te_comment);
  ...
  for (const llvm::Instruction& I : *B_to) {
    ...
    state_set_expr(state_out, expr_var_t::mk_expr_var_plain(mk_string_ref(phi_tmpvarname.at(varname))), val); //first update the tmpvars (do not want updates of one phi-node to influene the rhs of another phi-node.

    if (model_llvm_semantics) {
      //cout << _FNLN_ << ": val = " << expr_string(val) << endl;
      if (m_ctx->expr_is_input_expr(val)) {
        expr_var_ref const& val_key = val->expr_get_var()->expr_var_remove_input_prefix();
        ...
        if (set_belongs(m_poison_varnames_seen, val_key_poison)) {
          ...
          m_poison_varnames_seen.insert(expr_var_t::mk_expr_var_plain(mk_string_ref(phi_tmpvarname_poison.at(varname))));
          state_set_expr(state_out, expr_var_t::mk_expr_var_plain(mk_string_ref(phi_tmpvarname_poison.at(varname))), m_ctx->mk_var(val_key_poison->expr_var_add_input_prefix(), m_ctx->mk_bool_sort()));
        }
      }
    }

    dshared_ptr<tfg_node const> pc_to_phi_dst_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    ...
    //bbl_order_descriptor_t const& bbo = this->m_bbl_order_map.at(mk_string_ref(bbindex));
    auto e = mk_tfg_edge(pc_to_phi_node->get_pc(), pc_to_phi_dst_node->get_pc(), expr_true(m_ctx), state_out, state_assumes, {}, phi_node_to_te_comment(/*bbo, */inum, I));
    eimap.insert(make_pair(e, &I));
    t.add_edge(e);
  ...
  for (const auto &cvarname_sort : changed_varnames) {
    ...
    state state_out;
    state_set_expr(state_out, expr_var_t::mk_expr_var_plain(mk_string_ref(cvarname)), m_ctx->mk_var(string(G_INPUT_KEYWORD ".") + phi_tmpvarname_for_cvarname, cvarsort));

    if (model_llvm_semantics) {
      if (phi_tmpvarname_poison.count(cvarname)) {
        string cvarname_poison = get_poison_value_varname(cvarname);
        m_poison_varnames_seen.insert(expr_var_t::mk_expr_var_plain(mk_string_ref(cvarname_poison)));
        state_set_expr(state_out, expr_var_t::mk_expr_var_plain(mk_string_ref(cvarname_poison)), m_ctx->mk_var(string(G_INPUT_KEYWORD ".") + phi_tmpvarname_poison.at(cvarname), m_ctx->mk_bool_sort())); //first update the tmpvars (do not want updates of one phi-node to influene the rhs of another phi-node.
      }
    }
...
sym_exec_llvm::get_fun_names(llvm::Module const *M)
{
...
class next_sym_id_gen
{
  dshared_ptr<tfg_llvm_t const> m_tfg;
  map<string,symbol_id_t> m_name2sym_id;
  symbol_id_t m_max_sym_id = -1;
  symbol_id_t m_next_sym_id = 1;

public:
  next_sym_id_gen(dshared_ptr<tfg_llvm_t const> src_llvm_tfg)
  : m_tfg(src_llvm_tfg)
  {
    tie(m_max_sym_id, m_name2sym_id) = compute_name_to_sym_id_map(src_llvm_tfg);
  }

  symbol_id_t next(string const& name)
  {
    if (auto it = m_name2sym_id.find(name); it != m_name2sym_id.end()) {
      m_next_sym_id = it->second;
      return m_next_sym_id++;
    }
    if (m_tfg && m_tfg->get_symbol_map().count(m_next_sym_id)) {
      m_next_sym_id = m_max_sym_id + 1;
      return m_next_sym_id++;
    }
    if (m_tfg && m_tfg->get_extsym_map().count(m_next_sym_id)) {
      m_next_sym_id = m_max_sym_id + 1;
      return m_next_sym_id++;
    }
    return m_next_sym_id++;
  ...
  symbol_id_t get() const { return m_next_sym_id; }

private:
  static pair<symbol_id_t,map<string,symbol_id_t>>
  compute_name_to_sym_id_map(dshared_ptr<tfg_llvm_t const> src_llvm_tfg)
  {
    if (!src_llvm_tfg) {
      return {};
    }
    symbol_id_t max_sym_id = -1;
    map<string,symbol_id_t> name2sym_id;
    for (auto const& [sym_id,sym] : src_llvm_tfg->get_symbol_map().get_map()) {
      max_sym_id = max(sym_id, max_sym_id);
      bool inserted = name2sym_id.emplace(sym.get_name()->get_str(), sym_id).second;
      ASSERT(inserted);
    }
    for (auto const& [sym_id,sym] : src_llvm_tfg->get_extsym_map().get_map()) {
      max_sym_id = max(sym_id, max_sym_id);
      bool inserted = name2sym_id.emplace(sym.get_name()->get_str(), sym_id).second;
      ASSERT(inserted);
    }
    return {max_sym_id, name2sym_id};
  }
};

}

tuple<map<symbol_id_t,graph_symbol_t>, map<symbol_id_t,graph_extsym_t>, map<pair<symbol_id_t,offset_t>,vector<char>>>
sym_exec_llvm::get_symbol_map_and_string_contents(Module const *M, list<pair<string, unsigned>> const &fun_names, dshared_ptr<tfg_llvm_t const> src_llvm_tfg)
{
  map<symbol_id_t,graph_symbol_t> smap;
  map<symbol_id_t,graph_extsym_t> extsym_map;
  map<pair<symbol_id_t, offset_t>, vector<char>> scontents;

  next_sym_id_gen ng(src_llvm_tfg);
  for (auto const& g : M->globals()) {
    ...
    ASSERT(cv);
    uint64_t const symbol_size = dl.getTypeAllocSize(ElTy);
    unsigned const symbol_alignment = dl.getPreferredAlign(cv).value();
    bool const symbol_is_constant = cv->isConstant();
    auto const symbol_id = ng.next(varname->get_name()->get_str());

    DYN_DEBUG2(llvm2tfg_syms, cout << "Global name = " << varname->get_name()->get_str() << ", generated symbol_id = " << symbol_id << '\n');
    if (cv->hasExternalLinkage() && cv->isDeclaration()) {
      if (auto *arr = llvm::dyn_cast<llvm::ArrayType>(cv->getValueType());
        ...
        DYN_DEBUG2(llvm2tfg_syms, cout << "Added " << varname->get_name()->get_str() << " to extsym_map\n");
        bool inserted = extsym_map.emplace(symbol_id, graph_extsym_t{mk_string_ref(varname->get_name()->get_str()), symbol_alignment, symbol_is_constant}).second;
        ASSERT(inserted);
        // declaration won't have an initializer
        ASSERT(!cv->hasInitializer());
        continue;
      }
    }
    ...
    if (symbol_is_constant && cv->hasInitializer()) {
      scontents[make_pair(symbol_id, 0)] = get_constant_bytes(cv->getInitializer());
    }
  }
...
expr_var_ref
sym_exec_llvm::get_poison_value_varname(expr_var_ref const& varname) const
{
  stringstream ss;
  ss << varname->get_name()->get_str() << ".poison";
  //return ss.str();
  return varname->expr_var_set_name(ss.str());
}

//string
//sym_exec_llvm::get_poison_value_varname(string const& varname) const
//{
//  return varname + ".poison";
//}


expr_ref
sym_exec_llvm::get_poison_value_var(expr_var_ref const& varname) const
{
...
//{
//  return get_input_expr(get_poison_value_varname(varname), m_ctx->mk_bool_sort());
//}

void
sym_exec_llvm::transfer_poison_values(string const& varname, expr_ref const& e, preds_t& state_assumes, dshared_ptr<tfg_node const>& from_node, bool model_llvm_semantics, tfg& t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  if (!model_llvm_semantics) {
    return;
  }
  ...
  expr_visit_vars(e, [this,&poison_vars](expr_ref const& v) {
    ...
    expr_var_ref poison_varname = expr_var_t::mk_expr_var_plain(mk_string_ref(get_poison_value_varname(varname)));
  ...
  for (auto const& poison_var : poison_vars) {
    if (varname == "") {
      state_assumes.emplace_back(m_ctx->mk_not(poison_var), fails::safety_llvm_val_not_poison);
    } else {
      if (poison_expr) {
  ...
  if (varname != "" && poison_expr) {
    ...
    //}
    expr_var_ref poison_var = expr_var_t::mk_expr_var_plain(mk_string_ref(poison_varname));
    m_poison_varnames_seen.insert(poison_var);

    state state_to_intermediate_val;
    state_set_expr(state_to_intermediate_val, poison_var, poison_expr);
    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    shared_ptr<tfg_edge const> e = mk_tfg_edge(from_node->get_pc(), intermediate_node->get_pc(), expr_true(m_ctx), state_to_intermediate_val, {}, {}, te_comment_t(-1,-1,"poison"));
    t.add_edge(e);
    from_node = intermediate_node;
  }
}
...
void
sym_exec_llvm::add_state_assume(string const& varname, expr_with_fail const& assume, state const& state_in, preds_t& assumes, dshared_ptr<tfg_node const>& from_node, bool model_llvm_semantics, tfg& t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  if (model_llvm_semantics && varname != "") {
    ...
    if (set_belongs(m_poison_varnames_seen, poison_varname)) {
      expr_ref poison_var = get_input_expr(poison_varname, m_ctx->mk_bool_sort());
      poison_expr = m_ctx->mk_or(poison_var, assume.expr());
    } else {
      poison_expr = assume.expr();
    }
    ...
    dshared_ptr<tfg_node const> intermediate_node = get_next_intermediate_subsubindex_pc_node(t, from_node);
    shared_ptr<tfg_edge const> e = mk_tfg_edge(from_node->get_pc(), intermediate_node->get_pc(), expr_true(m_ctx), state_to_intermediate_val, {}, {}, te_comment_t(-1,-1,string("poison.")+assume.fail().to_string()));
    t.add_edge(e);
    from_node = intermediate_node;
  } else {
    if (!assume.expr()->is_const_bool_true()) {
      assumes.emplace_back(assume);
    }
  }
}

void
sym_exec_llvm::transfer_poison_value_on_load(expr_var_ref/*string*/ const& varname, expr_ref const& load_expr, preds_t& state_assumes, dshared_ptr<tfg_node const>& from_node, bool model_llvm_semantics, tfg& t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  if (!model_llvm_semantics) {
    return;
  }
  ...
  ASSERT(load_expr->get_args().at(OP_SELECT_ARGNUM_MEM)->get_name()->get_str() == string(G_INPUT_KEYWORD ".") + m_mem_reg->get_name()->get_str());

  string poison_var/*name*/ = get_poison_value_varname(varname);
  //expr_var_ref poison_var = expr_var_t::mk_expr_var_plain(mk_string_ref(poison_varname));
  m_poison_varnames_seen.insert(poison_var);
...
sym_exec_llvm::transfer_poison_value_on_store(expr_ref const& store_expr, preds_t& state_assumes, dshared_ptr<tfg_node const>& from_node, bool model_llvm_semantics, tfg& t, map<llvm_value_id_t, expr_var_ref>* value_to_name_map)
{
  ...
  ASSERT(store_expr->get_args().at(OP_STORE_ARGNUM_MEM)->get_name()->get_str() == string(G_INPUT_KEYWORD ".") + m_mem_reg->get_name()->get_str());
  ...
  expr_visit_vars(data, [this,&poison_expr](expr_ref const& v) {
    ...
    //expr_var_ref poison_vname = expr_var_t::mk_expr_var_plain(mk_string_ref(get_poison_value_varname(vname)));
    expr_var_ref poison_vname = get_poison_value_varname(v);
...
sym_exec_llvm::get_next_undef_varname()
{
  ...
  return expr_var_t::mk_expr_var_plain(mk_string_ref(ss.str()));
}
