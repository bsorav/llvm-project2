...
{
  bool found = false;
  AliasResult semanticAA_result(AliasResult::NoAlias), otherAA_result(AliasResult::NoAlias);
  otherAA_result = AliasResult::MayAlias;
  for (auto const& SAA : AAs) {
    ...
    if (SAA->isSemanticAA()) {
      ...
      if (otherAA_result == AliasResult::MayAlias && this_result != AliasResult::MayAlias) {
  ...
  if (semanticAA_result == AliasResult::MayAlias && otherAA_result != AliasResult::MayAlias) {
    string s;
    ...
    std::cout << rso.str();
  } else if (semanticAA_result != AliasResult::MayAlias && otherAA_result == AliasResult::MayAlias) {
    string s;
    raw_string_ostream rso(s);
